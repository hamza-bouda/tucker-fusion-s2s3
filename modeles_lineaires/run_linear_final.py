...
class ALSLipAdapter(ALSAdapter):
    """ALS v0 historique : mise à jour des D par UN pas de gradient au pas de
    Lipschitz calculé (au lieu de la pseudo-solution fermée), et β ADAPTATIF
    (β = β_factor·‖G‖/numel) — la version qui donnait les meilleurs résultats."""
    def __init__(self, ranks, scene_path, scene_key):
        super().__init__(ranks, scene_path, scene_key)
        self.name = 'Tucker-lin (ALS Lipschitz)'

    def predict(self, inputs):
        from modeles_lineaires.v0_lineaire_baseline_tucker_als import run_gscott_tucker
        from gstereo_tucker_hyperbench import build_psf_matrix
        t0 = time.time()
        hr_msi = np.array(inputs.hr_msi)
        lr_hsi = np.array(inputs.lr_hsi)
        srf = np.array(inputs.srf)
        ...
        Bh = build_psf_matrix(H_sp, r_sp, psf_kernel)
        Bw = build_psf_matrix(W_sp, r_sp, psf_kernel)
        IH, IW, I_C = np.eye(H_sp), np.eye(W_sp), np.eye(C)
        # format v0 : (X, [P1, P2, P3], lam) — source M en tête (init D1/D2/G),
        # source H en queue (init D3)
        sources = [(M1, [IH, IW, srf], 1.0), (H_n, [Bh, Bw, I_C], 1.0)]

        S, G, Ds, beta = run_gscott_tucker(
            sources, list(self._ranks_als), beta_factor=0.01,
            n_outer=30, n_fista=100, verbose=False)
        S = np.clip(S * s_H, 0.0, 1.0)
        stats = {'fit_time_s': time.time() - t0,
                 'sparsity_G': float(np.mean(np.abs(G) < 1e-8)) * 100,
                 'beta_adaptatif': float(beta)}
        self._add_q2n(S, stats)
        return S, stats
...
if __name__ == '__main__':
    ...
    for scene_tag, scene_path, scene_key in SCENES:
        methods = [
            ...
            ('als_lip', lambda: ALSLipAdapter(ranks, scene_path, scene_key)),
    ...
    for scene_tag, _, _ in SCENES:
        for tag in ('als', 'als_lip', 'adam_prox', 'adam_l1', 'sgd_prox', 'lbfgs_prox'):
            ...
            if (scene_tag, tag) not in results and os.path.exists(p):
                try:
                    ...
                    pass
    LABELS = {'als': 'ALS + FISTA', 'als_lip': 'ALS Lipschitz (v0)',
