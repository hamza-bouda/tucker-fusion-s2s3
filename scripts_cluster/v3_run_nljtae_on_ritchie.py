"""
v3_run_nljtae_on_ritchie.py
---------------------------------------------------------------------------
Téléversement et soumission du script d'entraînement du modèle v3 (ConvTranspose, Skips & SAM Loss)
...
def main():
    ...
    files_to_upload = [
        ...
        ("v1_autoencodeur_couple_ctae.py", f"{remote_dir}/v1_autoencodeur_couple_ctae.py")
    ]
    
    for local_file, remote_file in files_to_upload:
        print(f"Téléversement de {local_file} vers {remote_file}...")
        sftp.put(local_file, remote_file)
    print("Téléversement des fichiers terminé.")
...
#OAR -n NL_JTAE_V3_RITCHIE
#OAR -l /nodes=1/core=8,walltime=02:00:00
#OAR -p gpumodel='A100'
#OAR -O NL_JTAE_V3_%jobid%.out
#OAR -E NL_JTAE_V3_%jobid%.err

echo "Demarrage du job OAR NL-JTAE v3 sur GPU (Ritchie)"
hostname
nvidia-smi
...
python -u v3_non_lineaire_conjoint_nljtae.py --dataset PaviaU --epochs 50000 --ranks 16 16 32 --lr 2e-3
"""
    
    remote_submit_script = f"{remote_dir}/submit_nl_jtae_v3_ritchie.sh"
    ...
    stdin, stdout, stderr = ssh.exec_command(f"cd {remote_dir} && oarsub -S ./submit_nl_jtae_v3_ritchie.sh")
    ...
    if job_id:
        ...
        print("\n=== LECTURE DES LOGS ===")
        stdin, stdout, stderr = ssh.exec_command(f"cat ~/tucker_fusion/NL_JTAE_V3_{job_id}.out")
        log_out = stdout.read().decode('utf-8', errors='ignore')
        print(log_out if log_out.strip() else "Aucun log standard trouvé.")
        
        stdin, stdout, stderr = ssh.exec_command(f"cat ~/tucker_fusion/NL_JTAE_V3_{job_id}.err")
        ...
        try:
            ...
            sftp.get("tucker_fusion/v3_nljtae_comparaison_paviau.png", "v3_nljtae_comparaison_paviau.png")
            print("Figure rapatriée avec succès : v3_nljtae_comparaison_paviau.png")
            
            # Rapatrier aussi dans les artifacts
            shutil_dest = "C:\\Users\\hamza\\.gemini\\antigravity\\brain\\15201f33-80da-4d2c-a3f4-2b3479314927\\v3_nljtae_comparaison_paviau.png"
            import shutil
            shutil.copy("v3_nljtae_comparaison_paviau.png", shutil_dest)
            
            print("Rapatriement de l'image HSI de résultat...")
            os.makedirs("results", exist_ok=True)
            sftp.get("tucker_fusion/results/PaviaU_reconstructed_NL_JTAE_v3.tif", "results/PaviaU_reconstructed_NL_JTAE_v3.tif")
            print("Image HSI de résultat rapatriée avec succès : results/PaviaU_reconstructed_NL_JTAE_v3.tif")
