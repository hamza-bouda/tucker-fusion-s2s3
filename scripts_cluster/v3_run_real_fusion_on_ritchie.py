"""
v3_run_real_fusion_on_ritchie.py
---------------------------------------------------------------------------
Téléversement et soumission du script de fusion réelle (v3_real_fusion.py)
...
def main():
    ...
    files_to_upload = [
        ("v3_real_fusion.py", f"{remote_dir}/v3_real_fusion.py"),
    ...
    # 2. Téléverser les images satellites réelles s'il y a lieu
    local_data_dir = "./data"
...
#OAR -n REAL_FUSION_V3_RITCHIE
#OAR -l /nodes=1/core=8,walltime=02:00:00
#OAR -p gpumodel='A100'
#OAR -O REAL_FUSION_V3_%jobid%.out
#OAR -E REAL_FUSION_V3_%jobid%.err

echo "Demarrage de la fusion réelle Sentinel S2/S3 (v3) sur GPU"
hostname
nvidia-smi
...
python -u v3_real_fusion.py
"""
    
    remote_submit_script = f"{remote_dir}/submit_real_fusion_v3.sh"
    ...
    stdin, stdout, stderr = ssh.exec_command(f"cd {remote_dir} && oarsub -S ./submit_real_fusion_v3.sh")
    ...
    if job_id:
        ...
        print("\n=== LECTURE DES LOGS ===")
        stdin, stdout, stderr = ssh.exec_command(f"cat ~/tucker_fusion/REAL_FUSION_V3_{job_id}.out")
        log_out = stdout.read().decode('utf-8', errors='ignore')
        print(log_out if log_out.strip() else "Aucun log standard trouvé.")
        
        stdin, stdout, stderr = ssh.exec_command(f"cat ~/tucker_fusion/REAL_FUSION_V3_{job_id}.err")
        ...
        try:
            ...
            sftp.get("tucker_fusion/v3_real_fusion_comparaison.png", "v3_real_fusion_comparaison.png")
            print("Figure rapatriée avec succès : v3_real_fusion_comparaison.png")
            
            # Copie dans les artifacts
            shutil.copy("v3_real_fusion_comparaison.png", "C:\\Users\\hamza\\.gemini\\antigravity\\brain\\15201f33-80da-4d2c-a3f4-2b3479314927\\v3_real_fusion_comparaison.png")
            print("Figure copiée dans les artéfacts.")
            
            print("\nRapatriement de l'image super-résolue...")
            os.makedirs("results", exist_ok=True)
            sftp.get("tucker_fusion/results/real_super_resolved_v3.tif", "results/real_super_resolved_v3.tif")
            print("Image super-résolue rapatriée : results/real_super_resolved_v3.tif")
            
            print("\nRapatriement des poids du modèle...")
            sftp.get("tucker_fusion/results/real_NL_JTAE_v3_weights.pth", "results/real_NL_JTAE_v3_weights.pth")
            print("Poids du modèle rapatriés : results/real_NL_JTAE_v3_weights.pth")
