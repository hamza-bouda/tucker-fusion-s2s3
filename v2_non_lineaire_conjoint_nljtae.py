...
# ── ÉTAPE 2 : ARCHITECTURE 100% NON-LINÉAIRE CONJOINTE ───────────────────────
...
class NonLinearTuckerDecoder(nn.Module):
    def __init__(self, out_channels=103, in_channels=8, spatial_in=(16, 16), spatial_out=(240, 240)):
        super().__init__()
        # Le tenseur cœur G a la forme (B, R1, R2, R3)
        # On transpose en (B, R3, R1, R2) pour les convolutions spatiales 2D
        # R1, R2 = 16, 16. On passe à 240x240 en faisant x3 puis x5.
        ...
        self.final_conv = nn.Sequential(
            nn.Conv2d(128, out_channels, kernel_size=3, padding=1),
            nn.Sigmoid() # PaviaU est normalisée dans [0, 1]
        )
        
    def forward(self, G):
        # G: (B, R1, R2, R3) -> permute en (B, R3, R1, R2)
        x = G.permute(0, 3, 1, 2)
        x = self.block1(x)
        x = self.block2(x)
...
class NonLinearJointTuckerAE(nn.Module):
    ...
    def forward(self, x_joint):
        # x_joint: (B, 34, H, W)
        G = self.encoder(x_joint)
        
        # 1. Super-résolution HSI complète
        S_pred = self.decoder(G)  # (B, H, W, C)
        
        # 2. Projection physique S2 (13 bandes)
        # S_pred: (B, H, W, C) -> S_pred @ R_S2.T
        y_S2_pred_full = torch.matmul(S_pred, self.R_S2.T)
        
        # 3. Projection physique S3 (21 bandes à 300m)
        # Dégradation spatiale de S_pred (H, W, C) -> (H_3, W_3, C)
        # Puis multiplication spectrale @ R_S3.T -> (H_3, W_3, 21)
...
def run_experiment(dataset_name="PaviaU", ranks=(16, 16, 8), epochs=1000, lr=2e-3,
                   lam_super=1.0, lam_S2=1.0, lam_S3=1.0, lam_sparse=1e-6):
    
    print("\n" + "="*80)
    print(f" EXPÉRIENCE : NL-JTAE (100% Non-linéaire Conjoint) {dataset_name} | RANKS : {ranks} ")
    print("="*80)
    ...
    print("\nDébut de l'entraînement NL-JTAE...")
    t_start = time.time()
    ...
    for epoch in range(1, epochs + 1):
        ...
        # 4. Régularisation L1 sur le tenseur cœur unique
        loss_sparse = torch.mean(torch.abs(G))
        
        # Perte globale (pas de couple loss car le tenseur coeur est unique !)
        ...
        if epoch % 20 == 0 or epoch == 1:
            print(f"Epoch {epoch:03d}/{epochs} | Loss: {loss.item():.2e} (Super: {loss_super.item():.2e}, S2: {loss_S2.item():.2e}) | PSNR: {current_psnr:.2f} dB | SAM: {current_sam:.2f}° | G Sparsity: {sparsity:.1f}%")
    ...
    # ── ÉTAPE 4 : CALCUL DES MÉTRIQUES FINALES ──
    ...
    print("\n" + "="*50)
    print(" RÉSULTATS DE L'EVALUATION FINALE NL-JTAE ")
    print("="*50)
    ...
    axes[0, 1].set_title(f"Fused NL-JTAE (PSNR: {final_psnr:.1f}dB)", color='white')
    axes[0, 1].axis('off')
    ...
    axes[1, 2].axis('off')
    axes[1, 2].text(0.1, 0.8, f"Modèle: Non-Linear JTAE", color='white', fontsize=12, fontweight='bold')
    ...
    plt.tight_layout()
    plot_path = "./v2_nljtae_comparaison_paviau.png"
    plt.savefig(plot_path, dpi=150, facecolor='#0d1117')
    print(f"Figure sauvegardée sous : {plot_path}")
    
    # Rapatrier la figure
    import shutil
    shutil.copy(plot_path, "C:\\Users\\hamza\\.gemini\\antigravity\\brain\\15201f33-80da-4d2c-a3f4-2b3479314927\\v2_nljtae_comparaison_paviau.png")
    
    # Sauvegarde TIFF
    result_dest = "results/PaviaU_reconstructed_NL_JTAE.tif"
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Auto-encodeur Tucker Conjoint 100% Non-linéaire (NL-JTAE)")
